...
class OutboundLeadsController extends GetxController {
  final ApiService apiService = ApiService(dio: DioClient().dio);
  final leadData = Rxn<OutboundLeadsModel>();
  ...
  Future<void> fetchLeads({String? filter, String? startDate, String? endDate}) async {
    try {
      ...
      final response = await apiService.getOutboundLeads(
        filter: filter,
        startDate: startDate,
        endDate: endDate,
      );
      ...
      if (response.success == true && response.data != null) {
        ...
        leadData.value = OutboundLeadsModel(
          data: OutboundData(
  ...
  void onVVITap() {
    Get.toNamed(
      AppRoutes.outboundLeadsDetailScreen,
      arguments: {
        'status': 'Very Interested',
        'leads': leadData.value?.data?.veryInterested?.data ?? []
      },
    );
  }
  ...
  void onMayBeTap() {
    Get.toNamed(
      ...
      arguments: {
        'status': 'May Be',
        'leads': leadData.value?.data?.maybe?.data ?? []
      },
    );
  }
  ...
  void onEnrolledTap() {
    Get.toNamed(
      ...
      arguments: {
        'status': 'Enrolled',
        'leads': leadData.value?.data?.enrolled?.data ?? []
      },
    );
  }
}