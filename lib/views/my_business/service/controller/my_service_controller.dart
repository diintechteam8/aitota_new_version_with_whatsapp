...
class MyServiceController extends GetxController {
  final RxList<Map<String, dynamic>> serviceItems = <Map<String, dynamic>>[].obs;
  ...
  Future<void> fetchMyServices() async {
    try {
      ...
      if (response.success == true && response.data != null) {
        serviceItems.clear();
        serviceItems.addAll(response.data!.where((item) => item.type == 'service').map((business) {
          return {
            'id': business.sId ?? '',
            'title': business.title ?? 'Untitled Service',
        ...
        customSnackBar(message: "Failed to fetch services", type: "E");
      }
  ...
  Future<void> deleteMyBusiness(String businessId) async {
    try {
      ...
      if (response.success == true) {
        serviceItems.removeWhere((item) => item['id'] == businessId);
      } else {
