...
class InboundLeadStatusDetailScreen
    extends GetView<InboundLeadStatusDetailController> {
  const InboundLeadStatusDetailScreen({super.key});
...
// Mobile row
              ...
              SizedBox(height: 8.h), // Consistent gap
// Time row
              Row(
                children: [
                  Text(
...
// Dynamically build actionIcons based on whatsappRequested
    ...
    if (controller.lead.value?.metadata?.whatsappRequested ?? false) {
      actionIcons.add({
        ...
        'onTap': () {
          Get.toNamed(AppRoutes.inboundWhatsappChatScreen);
        },
      });
    }
